...
TEST_YAML_OVERRIDE = "tests/fixtures/config_mooncake_override.yaml"
...
def test_renders_config_override():
    # mooncake.config 覆写生效: 64GB / 9000
    r = subprocess.run(
        [sys.executable, "resolve/render_mooncake.py", "--config", TEST_YAML_OVERRIDE, "--output", OUT],
        capture_output=True, text=True,
    )
    assert r.returncode == 0, r.stderr
    with open(OUT) as f:
        cfg = json.load(f)
    assert cfg["global_segment_size"] == "64GB"
    assert cfg["default_kv_lease_ttl"] == 9000
    assert cfg["master_server_address"] == "10.0.0.1:50088"
