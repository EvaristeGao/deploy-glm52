...
> 基座：a2 inventory（8 节点，`ip` = 真实网卡 IP 172.16.0.x）。复现：`cd ansible && .venv/bin/python simulate_modes.py`
> 四模式 shell 命令全部 bash 语法通过。standalone 独立 haproxy 容器镜像 = `haproxy.image`（gwh-haproxy:v1.0）。
...
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p0]
    [run_dp_prefill_template.sh.j2] 3543 字符
...
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p3]
    [run_dp_prefill_template.sh.j2] 3542 字符
...
### 任务 2. 删除旧容器（幂等，对照 deploy.sh 的 docker rm -f $CONTAINER_NAME）
  执行于 [d0, d1, d2, d3, p0, p1, p2, p3]  bash-OK
    | docker rm -f standalone-haproxy-gwh-test 2>/dev/null || true

### 任务 3. 起容器（对照 deploy.sh start_node 的 docker run，无遗漏）
  执行于 [d0, d1, d2, d3, p0, p1, p2, p3]  bash-OK
    | docker run -itd --name standalone-haproxy-gwh-test \
    ...
    | -v /mnt/sfs_turbo/g00832294/certs:/etc/pki/etcd/certs \
...
### 任务 7. docker cp 启动文件进容器 /root/pd/（对照 deploy.sh start_node；mooncake.json 仅 kvpool） [{'name': 'launch_online_dp.py', 'only_kvpool': False}]
  执行于 [d0, d1, d2, d3, p0, p1, p2, p3]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /root/pd && docker cp /tmp/launch_online_dp.py standalone-haproxy-gwh-test:/root/pd/

### 任务 8. docker cp 启动文件进容器 /root/pd/（对照 deploy.sh start_node；mooncake.json 仅 kvpool） [{'name': 'mooncake.json', 'only_kvpool': True}]
  执行于 [d0, d1, d2, d3, p0, p1, p2, p3]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /root/pd && docker cp /tmp/mooncake.json standalone-haproxy-gwh-test:/root/pd/

### 任务 9. 起 etcd 容器（3 节点，证书挂载，双向证书认证；仅 kvpool+HA）
  执行于 [p0]  bash-OK
    | docker rm -f etcd-p0 2>/dev/null || true
    | docker run -d --name etcd-p0 --network host \
    |   -v /mnt/sfs_turbo/g00832294/certs:/certs \
    |   quay.io/coreos/etcd:v3.7.1-arm64 \
    |   etcd --name s1 --data-dir /etcd-data \
    |   --listen-client-urls https://0.0.0.0:2379 \
    |   --advertise-client-urls https://172.16.0.218:2379 \
    |   --listen-peer-urls https://0.0.0.0:2380 \
    |   --initial-advertise-peer-urls https://172.16.0.218:2380 \
    |   --initial-cluster s1=https://172.16.0.218:2380,s2=https://172.16.0.224:2380,s3=https://172.16.0.190:2380 \
  ...
  执行于 [p1]  bash-OK
    ...
    |   --listen-client-urls https://0.0.0.0:2379 \
    |   --advertise-client-urls https://172.16.0.224:2379 \
    |   --listen-peer-urls https://0.0.0.0:2380 \
    |   --initial-advertise-peer-urls https://172.16.0.224:2380 \
    |   --initial-cluster s1=https://172.16.0.218:2380,s2=https://172.16.0.224:2380,s3=https://172.16.0.190:2380 \
    |   --client-cert-auth --trusted-ca-file=/certs/ca.crt --cert-file=/certs/server.crt --key-file=/certs/server.key \
    |   --peer-client-cert-auth --peer-trusted-ca-file=/certs/ca.crt --peer-cert-file=/certs/server.crt --peer-key-file=/certs/server.key
  执行于 [p2]  bash-OK
    ...
    |   --listen-client-urls https://0.0.0.0:2379 \
    |   --advertise-client-urls https://172.16.0.190:2379 \
    |   --listen-peer-urls https://0.0.0.0:2380 \
    |   --initial-advertise-peer-urls https://172.16.0.190:2380 \
    |   --initial-cluster s1=https://172.16.0.218:2380,s2=https://172.16.0.224:2380,s3=https://172.16.0.190:2380 \
    |   --client-cert-auth --trusted-ca-file=/certs/ca.crt --cert-file=/certs/server.crt --key-file=/certs/server.key \
    |   --peer-client-cert-auth --peer-trusted-ca-file=/certs/ca.crt --peer-cert-file=/certs/server.crt --peer-key-file=/certs/server.key
...
### 任务 10. 起 mooncake 容器（HA：mooncake.ha.nodes；单主：proxy 节点；独立于 vllm-ascend，含 mooncake_master+haproxy+nc）
  执行于 [p0]  bash-OK
    ...
    |   -v /mnt/sfs_turbo/g00832294/certs:/etc/pki/etcd/certs \
    |   -v /mnt/sfs_turbo/g00832294/deploy-glm52/logs:/mnt/sfs_turbo/g00832294/deploy-glm52/logs \
...
### 任务 13. docker cp 脚本进 mooncake 容器并生成/启动 haproxy（CacheMaster 12379）
  执行于 [p0]  bash-OK
    ...
    |    export ETCD_SERVER_LIST="172.16.0.218:2379;172.16.0.224:2379;172.16.0.190:2379"; \
...
### 任务 14. docker cp 脚本进 vllm-ascend 容器并生成/启动 haproxy（P/D 引擎 12489）
  执行于 [d0, d1, d2, d3, p0, p1, p2, p3]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /home/ma-user/haproxy
    | docker cp /tmp/gen_haproxy_etcd.sh standalone-haproxy-gwh-test:/home/ma-user/haproxy/
    | docker cp /tmp/start-ha.sh standalone-haproxy-gwh-test:/home/ma-user/haproxy/
    | docker exec standalone-haproxy-gwh-test bash -c \
...
### 任务 15. 容器内起 mooncake_master（HA：enable_ha+etcd 选主；单主：直连）
  执行于 [p0]  bash-OK
    | docker exec mooncake-p0 mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    ...
    |     >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/mooncake_p0.log 2>&1'
    | sleep 2 && docker exec mooncake-p0 pgrep -x mooncake_master >/dev/null
  执行于 [p1]  bash-OK
    | docker exec mooncake-p1 mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec mooncake-p1 pkill -x mooncake_master 2>/dev/null || true
    | docker exec -d mooncake-p1 bash -c \
    |   'mooncake_master -enable_ha=true \
    |     --cluster_id=mooncake \
    |     --etcd_endpoints="127.0.0.1:12379" \
    |     --rpc-address 172.16.0.224 \
    |     -rpc_port=52050 \
    |     -metrics_port=52052 \
    |     -eviction_high_watermark_ratio=0.9 \
    |     -eviction_ratio=0.2 \
    ...
    |     -log_dir=/var/log/mooncake_master \
    |     -max_log_size=1800 -stderrthreshold=4 -stop_logging_if_full_disk=true \
    |     >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/mooncake_p1.log 2>&1'
    | sleep 2 && docker exec mooncake-p1 pgrep -x mooncake_master >/dev/null
  执行于 [p2]  bash-OK
    | docker exec mooncake-p2 mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec mooncake-p2 pkill -x mooncake_master 2>/dev/null || true
    | docker exec -d mooncake-p2 bash -c \
    |   'mooncake_master -enable_ha=true \
    |     --cluster_id=mooncake \
    |     --etcd_endpoints="127.0.0.1:12379" \
    |     --rpc-address 172.16.0.190 \
    |     -rpc_port=52050 \
    |     -metrics_port=52052 \
    |     -eviction_high_watermark_ratio=0.9 \
    |     -eviction_ratio=0.2 \
    ...
    |     -log_dir=/var/log/mooncake_master \
    |     -max_log_size=1800 -stderrthreshold=4 -stop_logging_if_full_disk=true \
    |     >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/mooncake_p2.log 2>&1'
...
### 任务 16. 渲染 run_dp_template.sh（每节点，dest 为节点 /tmp，模板用 inventory ip 字段）
  ([run_dp_decode_template.sh.j2] 3461 字符)
          ...
          | vllm serve /mnt/sfs_turbo/models/GLM-5.2-w4a8c8 \
...
### 任务 17. docker cp 模板进容器 /root/pd/run_dp_template.sh（对照 deploy.sh start_node）
  执行于 [d0]  bash-OK
    | docker cp /tmp/run_dp_template_d0.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [d1]  bash-OK
    | docker cp /tmp/run_dp_template_d1.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [d2]  bash-OK
    | docker cp /tmp/run_dp_template_d2.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [d3]  bash-OK
    | docker cp /tmp/run_dp_template_d3.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [p0]  bash-OK
    | docker cp /tmp/run_dp_template_p0.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [p1]  bash-OK
    | docker cp /tmp/run_dp_template_p1.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [p2]  bash-OK
    | docker cp /tmp/run_dp_template_p2.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh
  执行于 [p3]  bash-OK
    | docker cp /tmp/run_dp_template_p3.sh standalone-haproxy-gwh-test:/root/pd/run_dp_template.sh

### 任务 18. 启动引擎（对照 deploy.sh start_engines，docker exec -d 后台）
  执行于 [d0]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 8 --tp-size 4 --dp-size-local 2 \
    |   --dp-rank-start 0 --dp-address 172.16.0.71 \
    |   --dp-rpc-port 16600 --vllm-start-port 9900 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_d0.log 2>&1'
  执行于 [d1]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 8 --tp-size 4 --dp-size-local 2 \
    |   --dp-rank-start 2 --dp-address 172.16.0.71 \
    |   --dp-rpc-port 16600 --vllm-start-port 9900 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_d1.log 2>&1'
  执行于 [d2]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 8 --tp-size 4 --dp-size-local 2 \
    |   --dp-rank-start 4 --dp-address 172.16.0.71 \
    |   --dp-rpc-port 16600 --vllm-start-port 9900 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_d2.log 2>&1'
  执行于 [d3]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 8 --tp-size 4 --dp-size-local 2 \
    |   --dp-rank-start 6 --dp-address 172.16.0.71 \
    |   --dp-rpc-port 16600 --vllm-start-port 9900 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_d3.log 2>&1'
  执行于 [p0]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 4 --tp-size 8 --dp-size-local 1 \
    |   --dp-rank-start 0 --dp-address 172.16.0.218 \
    |   --dp-rpc-port 16591 --vllm-start-port 9081 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_p0.log 2>&1'
  执行于 [p1]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 4 --tp-size 8 --dp-size-local 1 \
    |   --dp-rank-start 1 --dp-address 172.16.0.218 \
    |   --dp-rpc-port 16591 --vllm-start-port 9081 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_p1.log 2>&1'
  执行于 [p2]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 4 --tp-size 8 --dp-size-local 1 \
    |   --dp-rank-start 2 --dp-address 172.16.0.218 \
    |   --dp-rpc-port 16591 --vllm-start-port 9081 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_p2.log 2>&1'
  执行于 [p3]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /mnt/sfs_turbo/g00832294/deploy-glm52/logs 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'cd /root/pd && python3 launch_online_dp.py \
    |   --dp-size 4 --tp-size 8 --dp-size-local 1 \
    |   --dp-rank-start 3 --dp-address 172.16.0.218 \
    |   --dp-rpc-port 16591 --vllm-start-port 9081 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/vllm_p3.log 2>&1'
...
### 任务 21. docker cp proxy 脚本进容器并启动（对照 deploy.sh start_proxy，pkill 锚定 ^python3）
  执行于 [p0]  bash-OK
    | docker exec standalone-haproxy-gwh-test mkdir -p /root/pd 2>/dev/null || true
    | docker cp /tmp/load_balance_proxy_server_example.py standalone-haproxy-gwh-test:/root/pd/
    | docker exec standalone-haproxy-gwh-test chmod +x /root/pd/load_balance_proxy_server_example.py
    | docker exec standalone-haproxy-gwh-test pkill -f '^python3 .*load_balance_proxy_server_example' 2>/dev/null || true
    | docker exec -d standalone-haproxy-gwh-test bash -c 'unset http_proxy https_proxy; python3 /root/pd/load_balance_proxy_server_example.py \
    |   --host 0.0.0.0 --port 1999 \
    |   --prefiller-hosts 172.16.0.218 172.16.0.224 172.16.0.190 172.16.0.34 --prefiller-ports 9081 9081 9081 9081 \
    |   --decoder-hosts 172.16.0.71 172.16.0.71 172.16.0.108 172.16.0.108 172.16.0.198 172.16.0.198 172.16.0.204 172.16.0.204 --decoder-ports 9900 9901 9900 9901 9900 9901 9900 9901 >> /mnt/sfs_turbo/g00832294/deploy-glm52/logs/proxy_{node}.log 2>&1'
...
- 渲染 mooncake.json（kvpool HA：master 经 etcd://haproxy 发现）
    ...
    |   "protocol": "ascend",
    |   "device_name": "",
    |   "master_server_address": "etcd://172.16.0.218:12379;172.16.0.224:12379;172.16.0.190:12379",
    |   "global_segment_size": "80GB",
    |   "default_kv_lease_ttl": 11000
    | }
- [跳过] 渲染 mooncake.json（kvpool 单主：master 直连 p0 rpc 端口，待实机确认）
...
### 任务 15. 容器内起 mooncake_master（HA：enable_ha+etcd 选主；单主：直连）
  执行于 [p0]  bash-OK
    ...
    |   'mooncake_master -enable_ha=true \
    |     --cluster_id=mooncake \
    |     --etcd_endpoints="172.16.0.218:12379;172.16.0.224:12379;172.16.0.190:12379" \
    |     --rpc-address 172.16.0.218 \
    |     -rpc_port=52050 \
    |     -metrics_port=52052 \
    |     -eviction_high_watermark_ratio=0.9 \
...
- 渲染 mooncake.json（kvpool 单主：master 直连 p0 rpc 端口，待实机确认）
    ...
    |   "protocol": "ascend",
    |   "device_name": "",
    |   "master_server_address": "172.16.0.218:52050",
    |   "global_segment_size": "80GB",
    |   "default_kv_lease_ttl": 11000
    | }
...
- 确保 ansible/generated/ 目录存在（generated/ 在 .gitignore，首次运行需创建）：(非命令任务，跳过渲染)
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [d0]
    [run_dp_decode_template.sh.j2] 2850 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [d1]
    [run_dp_decode_template.sh.j2] 2851 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [d2]
    [run_dp_decode_template.sh.j2] 2851 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [d3]
    [run_dp_decode_template.sh.j2] 2851 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p0]
    [run_dp_prefill_template.sh.j2] 2972 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p1]
    [run_dp_prefill_template.sh.j2] 2972 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p2]
    [run_dp_prefill_template.sh.j2] 2972 字符
- 渲染各节点 run_dp_template.sh 到 ansible/generated/（对照 deploy.sh cmd_gen 的 render_template 循环） [p3]
    [run_dp_prefill_template.sh.j2] 2971 字符
- [跳过] 渲染 mooncake.json（kvpool HA：master 经 etcd://haproxy 发现）
- [跳过] 渲染 mooncake.json（kvpool 单主：master 直连 p0 rpc 端口，待实机确认）
