...
运行：`cd /home/gao/code/script/deploy-glm52 && uv run pytest tests/test_resolve_node.py -v`
...
def parse(cfg):
    ...
    # 多值列表（HOSTS/PORTS）输出带引号，便于 deploy.sh eval；测试剥掉引号再切分
    return {k: v.strip('"') for line in out.strip().splitlines()
...
运行：`cd /home/gao/code/script/deploy-glm52 && uv run pytest tests/test_resolve_router.py -v`
预期：FAIL / ERROR。
...
运行：`cd /home/gao/code/script/deploy-glm52 && uv run pytest tests/test_resolve_instances.py -v`
预期：FAIL / ERROR。
...
运行：`cd /home/gao/code/script/deploy-glm52 && uv run pytest tests/test_render_mooncake.py -v`
预期：FAIL / ERROR。
...
注：`$(date +%s%N)` 提供纳秒时间戳，在 GNU date 下可用；macOS 需换用 `date +%s` 的整秒差。部署机为 Linux，可接受。
...
运行：`cd /home/gao/code/script/deploy-glm52 && uv run pytest tests/ -v && bash -n deploy.sh && bash -n verify.sh && bash -n func_check.sh && bash -n templates/run_dp_prefill_template.sh && bash -n templates/run_dp_decode_template.sh`
